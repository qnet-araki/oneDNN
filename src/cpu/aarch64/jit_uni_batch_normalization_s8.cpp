...
#undef LDR_PARAM
    ...
    void compute_vscaleshift(const ZReg &vscale, const ZReg &vshift,
        ...
        xa_->fsqrt(vsqrtvar.s, p_512 / Xbyak_aarch64::T_m, vsqrtvar.s);
        ...
        if (pd_->use_scaleshift()) {
            ...
            fmls(vshift.s, p_512 / Xbyak_aarch64::T_m, vmean.s, vscale.s);
        } else {
    ...
    void compute_dst(bool need_tail = false) override {
        ...
        L(c_loop);
        {
            ...
            L(mb_sp_loop);
            {
                ...
                scvtf(v.s, p_512 / Xbyak_aarch64::T_m, v.s);

                fmad(v.s, p_512, vscale.s, vshift.s);
                if (with_relu_) uni_fmax(v, v, vzero);

                frinti(v.s, p_512 / Xbyak_aarch64::T_m, v.s);
                fcvtzs(v.s, p_512 / Xbyak_aarch64::T_m, v.s);
                if (need_tail) {
