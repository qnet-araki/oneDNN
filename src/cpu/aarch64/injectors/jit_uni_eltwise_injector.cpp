...
void jit_uni_eltwise_injector_f32<isa>::injector_preamble(
    ...
    if (save_state_) {
        ...
        if (preserved_vecs_count)
            h->sub_imm(h->X_SP, h->X_SP, preserved_vecs_count * vlen, x_tmp_0);
...
void jit_uni_eltwise_injector_f32<isa>::injector_preamble_tail(
    ...
    if (save_state_) {
        /* This route has not been tested */
        if (idx_off) h->add_imm(h->X_SP, h->X_SP, idx_off * vlen, x_tmp_0);
...
void jit_uni_eltwise_injector_f32<isa>::gelu_tanh_compute_vector_fwd(
    ...
    h->sub_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);

    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->str(ZReg(IDX(vmm_aux0)), ptr(x_tmp_0));

    // compute tanh(G(x))
    tanh_compute_vector_fwd(vmm_src);

    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->ldr(ZReg(IDX(vmm_aux0)), ptr(x_tmp_0));
    h->add_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);
...
void jit_uni_eltwise_injector_f32<isa>::swish_compute_vector_fwd(
    ...
    h->sub_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);
    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->str(ZReg(IDX(vmm_src)), ptr(x_tmp_0));
    // x*alpha
    h->fmul(vmm_src, vmm_src, ZRegS(IDX(table_val(alpha))));
    // sigmoid(x*alpha)
...
void jit_uni_eltwise_injector_f32<isa>::log_compute_vector_fwd(
    ...
    auto gather_table_values = [&](const TRegS &vmm_dst, const TRegS &vmm_idxs,
        ...
        h->add_imm(x_addr, x_table, table_start_idx + offt, x_tmp_1);

        h->mov(ZRegD(IDX(z_tmp)), ZRegD(IDX(vmm_idxs)));
        h->mul(z_tmp, 4);

        h->ld1w(z_tmp, p_mask / T_z, ptr(x_addr, z_tmp, SXTW));
    ...
    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->ldr(ZReg(IDX(vmm_aux1)), ptr(x_tmp_0));
    h->add_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);
...
void jit_uni_eltwise_injector_f32<isa>::pow_compute_vector_fwd(
    ...
    if (beta_ == -1) { // alpha / x
        ...
        h->sub_imm(h->X_SP, h->X_SP, n_gprs_to_save * gpr_size, x_tmp_0);
        for (size_t i = 0; i < n_gprs_to_save; ++i) {
            h->add_imm(x_tmp_0, h->X_SP, i * gpr_size, x_tmp_1);
            h->str(XReg(IDX(gprs_to_save[i])), ptr(x_tmp_0));
        }

        // caller obligation to save k-regs as callee may use them
        size_t n_k_regs_to_save = 8;
        h->sub_imm(h->X_SP, h->X_SP, n_k_regs_to_save * k_mask_size, x_tmp_0);
        for (size_t i = 0; i < n_k_regs_to_save; ++i) {
            h->add_imm(x_tmp_0, h->X_SP, i * k_mask_size, x_tmp_1);
            h->str(PReg(static_cast<uint32_t>(i)), ptr(x_tmp_0));
        }
        ...
        // `host_isa::vecs_count`.
        h->sub_imm(h->X_SP, h->X_SP, (vecs_count + 2) * vlen, x_tmp_0);

        for (size_t i = 2; i < vecs_count + 2; ++i) {
            h->add_imm(x_tmp_0, h->X_SP, i * vlen, x_tmp_1);
            h->str(ZReg(IDX(TRegS(i - 2))), ptr(x_tmp_0));
        }
        ...
        h->add_imm(x_tmp_0, h->X_SP, vlen, x_tmp_1);
        h->str(ZReg(IDX(vmm_src)), ptr(x_tmp_0));
        ...
        for (size_t i = 0; i < vlen / sizeof(float); ++i) {
            h->add_imm(x_tmp_0, h->X_SP, i * sizeof(float), x_tmp_1);
            h->add(x_tmp_0, x_tmp_0, h->x0);
            h->ld1(VReg(IDX(xmm0)).s[0], ptr(x_tmp_0));
            h->mov(z_tmp, 0);
            ...
            h->add_imm(x_tmp_0, h->X_SP, vlen, x_tmp_1);
            h->add(x_tmp_0, x_tmp_0, h->x0);
            h->ld1(VReg(IDX(xmm1)).s[0], ptr(x_tmp_0));
            h->mov(z_tmp, 0);
            for (int ii = 1; ii < 4; ii++) {
            ...
            h->add_imm(x_tmp_0, h->X_SP, i * sizeof(float), x_tmp_1);
            h->add(x_tmp_0, x_tmp_0, h->x0);
            h->st1(VReg(IDX(xmm0)).s[0], ptr(x_tmp_0));
        }
        ...
        for (size_t i = vecs_count + 1; i >= 2; --i) {
            h->add_imm(x_tmp_0, h->X_SP, i * vlen, x_tmp_1);
            h->ldr(ZReg(IDX(TRegS(i - 2))), ptr(x_tmp_0));
        }

        h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
        h->ldr(ZReg(IDX(vmm_src)), ptr(x_tmp_0));

        h->add_imm(h->X_SP, h->X_SP, (vecs_count + 2) * vlen, x_tmp_0);
        // restore k registers

        for (int i = n_k_regs_to_save - 1; i >= 0; --i) {
            h->add_imm(x_tmp_0, h->X_SP, i * k_mask_size, x_tmp_1);
            h->ldr(PReg(static_cast<uint32_t>(i)), ptr(x_tmp_0));
        }
        h->add_imm(h->X_SP, h->X_SP, n_k_regs_to_save * k_mask_size, x_tmp_0);

        // restore gpr registers
        for (int i = n_gprs_to_save - 1; i >= 0; --i) {
            h->add_imm(x_tmp_0, h->X_SP, i * gpr_size, x_tmp_1);
            h->ldr(gprs_to_save[i], ptr(x_tmp_0));
        }
        h->add_imm(h->X_SP, h->X_SP, n_gprs_to_save * gpr_size, x_tmp_0);
        h->fmul(vmm_src, vmm_src, ZRegS(IDX(table_val(alpha))));
    }
}
...
void jit_uni_eltwise_injector_f32<isa>::gelu_tanh_compute_vector_bwd(
    ...
    h->sub_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);
    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->str(ZReg(IDX(vmm_aux2)), ptr(x_tmp_0));

    // T = tanh(G1(x))
    tanh_compute_vector_fwd(vmm_src);

    h->add_imm(x_tmp_0, h->X_SP, 0, x_tmp_1);
    h->ldr(ZReg(IDX(vmm_aux2)), ptr(x_tmp_0));
    h->add_imm(h->X_SP, h->X_SP, vlen, x_tmp_0);
