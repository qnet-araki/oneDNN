...
void jit_sve_512_x8s8s32x_fwd_kernel::store_output(
...
#else
                        xa_->mov(zmm_tmp3.s, mask_tmp / T_m, zregs_aux[1].s);
                    }
    ...
    void jit_sve_512_x8s8s32x_fwd_kernel::compute_ker_dw(int ur_w, int pad_l,
        ...
        auto input_offset3 = [=](int oi, int ci, int ki) {
            return jcp.typesize_in
                    * input_offset2(input_spatial_index(oi, ki), ci);
        };

        auto kernel_offset = [=](int ci, int ki) {
            return jcp.typesize_in
                    * ((ci * jcp.kh * jcp.kw + ki) * jcp.ch_block);
        };
        ...
        for (int ci = 0; ci < jcp.nb_ch_blocking; ci++) {
            ...
            for (int ki = 0; ki < jcp.kw; ki++) {
                ...
                if (jcp.is_fast_depthwise) {
                    ...
                    xa_->mov(zmm_wei.b, kblend_mask / Xbyak_aarch64::T_m,
                            zmm_wei.b);
		...
		auto zmm_tmp = ZReg(30);
                ...
                auto zmm_tmp = ZReg(17);
                    ...
                    zip1(zmm_tmp.b, zmm_tmp.b, zmm_tmp.b);
                    zip1(zmm_tmp.h, zmm_tmp.h, zmm_tmp.h);
                    sxtb(zmm_wei.s, mask_all_one / Xbyak_aarch64::T_m,
                            zmm_tmp.s);
#if 0
		ldr(zmm_tmp, Xbyak_aarch64::ptr(reg_stack));
                xa_->add(reg_stack, reg_stack, 64);
    ...
    void jit_sve_512_x8s8s32x_fwd_kernel::compute_ker(int ur_w, int pad_l,
            int pad_r, ic_block_t last_ic_block_flag, bool h_padded) {
        if (jcp.is_depthwise)
            return compute_ker_dw(
                    ur_w, pad_l, pad_r, last_ic_block_flag, h_padded);
    ...
    void jit_sve_512_x8s8s32x_fwd_kernel::kh_loop(
        ...
        Label f_overflow_label, no_f_overflow_label, d_h_f_overflow_label,
                ...
                no_b_overflow_label, back_overflow_label,
                no_back_overflow_label, d_h_back_overflow_label;

        int ch_block_all = jcp.ch_block * jcp.ic_block * jcp.oc_block;
        int shift_kernel_ptr = jcp.typesize_in * jcp.kw * ch_block_all;
        int shift_input_ptr = jcp.typesize_in * jcp.iw * jcp.ic_without_padding
                * jcp.ngroups;
        ...
        if (jcp.ndims == 5) {
            ...
            if (!jcp.signed_input) {
                ...
                ldr(reg_ki,
                        Xbyak_aarch64::ptr(reg_param1, GET_OFF(f_overflow)));
                ...
                L(f_overflow_label);
                {
                    ...
                    {
                        compute_ker(
                                ur_w, pad_l, pad_r, last_ic_block_flag, true);
        ...
        if (!jcp.signed_input && jcp.ndims > 3) {
            ldr(reg_overflow,
                    Xbyak_aarch64::ptr(reg_param1, GET_OFF(t_overflow)));
        ...
        if (!jcp.signed_input && jcp.ndims > 3) {
            ldr(reg_overflow,
                    Xbyak_aarch64::ptr(reg_param1, GET_OFF(b_overflow)));
            xa_->cmp_imm(reg_overflow, 0, reg_tmp0_imm);
        ...
        if (jcp.ndims == 5) {
            adds_imm(aux_reg_inp_d, aux_reg_inp_d,
                    shift_input_ptr * jcp.ih * (jcp.dilate_d + 1),
                    reg_tmp0_imm);
            ...
            if (!jcp.signed_input) {
                ldr(reg_ki,
                        Xbyak_aarch64::ptr(reg_param1, GET_OFF(back_overflow)));
                xa_->cmp_imm(reg_ki, 0, reg_tmp0_imm);
    ...
    void jit_sve_512_x8s8s32x_fwd_kernel::generate() {
        ...
        int in_ic_shift = jcp.is_fused_conv
                ? jcp.dw_conv_buffer_oc
                : jcp.ic_without_padding * jcp.ngroups;
        int inp_shift_pad = jcp.typesize_in
                * (jcp.ur_w * jcp.stride_w - jcp.l_pad) * in_ic_shift;
        ...
        if (jcp.is_fused_conv) {
            ldr(reg_inp_buffer_ptr,
                    Xbyak_aarch64::ptr(reg_param1, GET_OFF(src)));
        ...
        */
        ...
        if (jcp.ngroups % jcp.ch_block != 0
            ...
            ldr(reg_oc_blocks,
                    Xbyak_aarch64::ptr(reg_param1, GET_OFF(oc_blocks)));
        ...
        if (jcp.nb_ow == 1) {
            ...
            bool next_last_ow_block_padded
                    = r_pad1 > 0 && n_oi_last_ow_block == 0;
            bool first_ow_block_padded
                    = next_last_ow_block_padded && jcp.nb_ow == 2;
            bool last_ow_block_padded = (r_pad1 > 0 || jcp.ur_w_tail == 0)
                    && n_oi_last_ow_block > 0;
            ...
            xa_->cmp_imm(
                    reg_owb, 0, reg_tmp0_imm); // is that the first ow-block ?
            ...
            if (jcp.l_pad > 0) {
                ...
                adds_imm(reg_inp, reg_inp, inp_shift_pad_second_block,
                        reg_tmp0_imm);
            }

            // set number of iteration for oi-loop
            if (n_oi_last_ow_block != n_oi_not_last_ow_block) {
                xa_->cmp_imm(reg_owb, jcp.nb_ow - 1,
                        reg_tmp0_imm); // last ow-block ?
            ...
            xa_->mov_imm(
                    reg_oi, n_oi_not_last_ow_block); // other middle ow-blocks
            ...
            ldr(reg_owb, Xbyak_aarch64::ptr(reg_param1, GET_OFF(owb)));
            xa_->cmp_imm(
                    reg_owb, jcp.nb_ow - 1, reg_tmp0_imm); // last ow_block?
            b(LT, end_label);
    ...
    status_t jit_sve_512_x8s8s32x_fwd_kernel::init_conf(jit_conv_conf_t & jcp,
        ...
        jcp.is_depthwise
                = true && with_groups && everyone_is(1, jcp.ic, jcp.oc);
        ...
        if (jcp.dst_zero_point || jcp.src_zero_point)
            return status::unimplemented;
    ...
    ch_block (= 16) would require byte masking for load from src */
        ...
        if (jcp.is_depthwise) {
            jcp.max_regs_ur = 31 - jcp.is_fast_depthwise
                    - !jcp.is_resrc_depthwise - !jcp.signed_input
        ...
        auto set_or_check_wei_format = [&]() {
            ...
            if (jcp.ic_block == 16 || jcp.ch_block == 16) {
                ...
                assert(with_groups);
                wei_tag = is_3d ? gOIdhw2i8o4i
                                : is_2d ? gOIhw2i8o4i : gOIw2i8o4i;
            } else {
        ...
        format_tag_t dat_tag = utils::pick(ndims - 3, format_tag::nwc,
        ...
        for (; jcp.nb_oc_blocking_thr_chunk > 1;
                jcp.nb_oc_blocking_thr_chunk--) {
        ...
        if (!jcp.is_depthwise && jcp.ur_w < jcp.ow) {
            ...
            if (jcp.ic_without_padding % broadcast_size != 0) {
                while (jcp.ur_w > 0) {
                    ...
                    int penultimate_iw_leeway
                            = (jcp.iw - 1 - penultimate_iw_index)
        ...
        for (int nb_ow = 1; nb_ow <= max_nb_ow; nb_ow++) {
            int ow_block = nstl::min(
                    rnd_up(div_up(jcp.ow, nb_ow), jcp.ur_w), jcp.ow);
        ...
        bool args_ok
                = true && jcp.oc % jcp.oc_block == 0 && jcp.l_pad <= jcp.ur_w;
